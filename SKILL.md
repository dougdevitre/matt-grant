...
description: >
  ...
  to win MO-02, fundraising via WinRed, volunteer and field operations, GOTV, and
  messaging. It also retains the full general get-elected capabilities for any
  U.S. campaign: deciding whether to run, filing for candidacy, setting up a
  committee, recruiting a team, raising money legally, tracking contributions and
  expenditures, checking donor limits, generating campaign finance disclaimers,
  preparing compliance filings, building fundraising and GOTV plans, drafting
  messaging, debate prep, press releases, and winding down a campaign for any
  federal, state, county, or municipal race. Trigger for ANY question about
  Matt Grant's campaign or about running for office, campaign strategy, campaign
  finance, political fundraising, FEC rules, state election law, PACs, Super PACs,
  527s, dark money, campaign compliance, donor limits, filing deadlines, ballot
  access, petition requirements, campaign treasurer duties, or post-election
  obligations. Also trigger for "Matt Grant", "MO-02", "Matt Grant for Congress",
  "how do I run for office", "can I accept this donation", "what are the
  contribution limits", "when is my filing deadline", "help me build a campaign",
  or any mention of running for city council, school board, state legislature,
  Congress, or any elected position. Always trigger when the user mentions
  "Matt Grant for Congress" or "get elected" by name.
---
...
**Candidate files (Matt Grant — load first for anything about this campaign):**
...
| `candidate/sign-placement-plan.md` | User asks where to place yard/corridor/polling-place signs for MO-02, how to score or allocate sign inventory across the early-vote and Election-Day windows, how to organize sign field turf, or the Missouri sign-placement / electioneering compliance rules for the Aug 4, 2026 primary — includes phased field ops, a scoring model, data schemas, and §9 compliance (25-ft poll buffer, right-of-way ban, private-property permission, per-municipality removal). Counts and weights are illustrative placeholders |
| `candidate/poll-coverage-plan.md` | User asks how to staff/cover the voting sites in MO-02 for the Aug 4, 2026 primary — greeters at early-vote sites and precinct polls, coverage prioritization, shift staffing, and the at-the-poll compliance rules (25-ft electioneering buffer, greeter vs. credentialed poll-watcher). The field-coverage companion to the sign plan; defers ballot-integrity/poll-watching law to `tactics/election-protection.md`. Counts are illustrative placeholders |
| `candidate/voter-file-plan.md` | User asks about the MO-02 voter file/list, voter data custody or legality (RSMo 115.157, TCPA), voter scoring/segments (propensity, support IDs, MOBILIZE/PERSUADE/BANK), walk/call/mail list generation from the file, or the voter-engine dashboard, or **exactly how/where voter records are used across the system** (usage & lineage, the TCPA code-enforced wall) — the governing doc for the real 577k-voter file: what it contains (and doesn't — no phones/emails, last-election-only history), S3 custody, sharded ingest architecture, honest scorecard, per-channel rules, and the §7 per-consumer usage & lineage map |
| `candidate/donor-value-ladder.md` | User asks about donor recognition tiers, giving levels, donor perks/premiums/schwag, what donors get at each amount, or how to thank donors within FEC rules — the $25→$7,000 recognition ladder (stickers → yard sign → tee → kit → reception → Founders Club → roundtable with Matt) with §3 compliance gates (full amount counts against limits, $3,500/election + $7,000 designation, premiums as committee fundraising expenses, no raffles, access ≠ official action) and fulfillment ops |
| `candidate/twilio-fund-plan.md` | User asks about the best use of Twilio/SMS/text-messaging funds, how to allocate the texting budget, which contacts/voters to text, or the TCPA line on texting the voter file — a decision brief that starts from the hard constraint (SMS only to opted-in numbers; the voter file is never texted), measures the opted-in audience joined to voter scores/segments, recommends a priority budget allocation (GOTV core first, MONITOR none), and points to `web/scripts/twilio-fund-report.ts` for live numbers. All dollar/count figures are illustrative placeholders |
| `candidate/sms-targeting-plan.md` | User asks how to TARGET SMS broadcasts at the highest-value people by voting data, prioritize/rank the texting audience by voter segment or turnout score, GOTV-suppress already-voted numbers, or wire voter scores into the SMS composer without breaching the TCPA wall — the send-side plan: enrich opted-in numbers with denormalized voter tags out-of-band, then filter/rank in the composer (never reading the voter file). Phase-3 ranking core shipped in `web/lib/reports/smsTargeting.ts` (`buildSendList`); Phases 1–2, 4–5 planned |
| `candidate/voter-registry-refresh-plan.md` | User asks about loading a NEW or UPDATED voter list, a purchased/vendor/party-committee voter export, a second voter-file source, "seeding" the voter registry, primary vote history or primary-propensity scoring, targeting an SMS blast by party or primary history, or whether a vendor file's phone numbers can be texted — the second-source plan: why a vendor file is an OVERLAY (`VOTEROVL`) and never the registration spine, the Excel 1,048,576-row truncation trap on a `.csv.xlsx` re-save, why any Missouri party value is **inferred, never registered**, the inspect → ingest → enrich toolchain, the new `pp:`/`party:` composer chips, and the operational limits on a blast (9am–8pm CT window, ~30 msg/min, ~10–13k recipients per campaign). The TCPA wall is unchanged: vendor phones are manual-dial/P2P only |
