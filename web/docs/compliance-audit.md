...
| 2026-06-18 | P-7 | Accessibility (WCAG 2.1 AA): fixed footer legal/disclaimer contrast (`text-paper/55`→`/70`, ~4.3→6.5:1); added `aria-label` to MediaLibrary videos (caption `.vtt` files still TODO for 1.2.2 — documented in code); resolved a systematic accent-contrast failure by adding a `goldlight` on-dark accent token and swapping ~12 on-navy `text-gold`/badge instances (home, about, footer) — the 2.98:1 accent now ≥5.7:1 on dark, unchanged on white. **Verified with axe-core 4.10**: all 13 public routes (home, about, issues, issue-detail, media, act, press, print, contact, donate, data-policy, transparency, public-trust) return **0 violations** across wcag2a/2aa/21a/21aa. Dashboard (staff-only) scanned in demo mode: main page 0 violations; fixed the one real finding — 6 unlabeled `<select>`s (critical `select-name`, WCAG 4.1.2) on finance/donors/tasks/volunteers/targets now have `aria-label`s. *(Two spurious `document-title`/`html-has-lang` hits during the dashboard scan were a hijacked browser tab from the concurrent process, not the app — root layout has `lang="en"` + title.)* |
