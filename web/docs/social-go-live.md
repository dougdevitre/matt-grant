...
| **LinkedIn** | `https://YOUR_DOMAIN/api/social/callback/linkedin` | `openid profile w_member_social` | Posting **as a person** works with these scopes. Posting **as an Organization/Page** also needs LinkedIn's **Community Management API** product + the `w_organization_social` scope approved (widen via `SOCIAL_LINKEDIN_SCOPES`). Set `LINKEDIN_AUTHOR_URN` (step 3). The campaign's app (Client ID `78lz8h5laq8pja`) also backs Clerk login — full walkthrough in [`linkedin-setup.md`](./linkedin-setup.md). |
