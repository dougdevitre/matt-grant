...
// Re-export so existing importers keep working; the fetch itself lives in its own
// module to avoid an import cycle with brandFonts.
export { googleFont } from "@/lib/googleFont";
...
export async function renderOgCard(card: OgCard): Promise<ImageResponse> {
  ...
  const [fonts, avatar] = await Promise.all([
    brandFonts(headlineText, bodyText),
