...
// Congress.gov record. Surfaces bills in the field's public legislative record that
// touch Matt's #1 priority (family-court corruption) and the CHILD Act's Title IV-D
...
//
// Lifted out of app/api/research/child-act/route.ts so the route AND the dashboard
// read view (app/dashboard/data/[id]) share one implementation.
...
import { type Resource, type Provenance, ok, degraded } from "@/lib/data/resource";

export type ChildActSubject = { slug: string; name: string; relation: string };
export type RankedBill = {
  ...
  subjects: ChildActSubject[];
};
export type ChildActView = {
  generatedAt: string;
  configured: boolean;
  count: number;
  byTier: { core: number; related: number; tangential: number };
  bills: RankedBill[];
  note: string;
};
...
export async function loadChildActBills(): Promise<Resource<ChildActView>> {
  const generatedAt = new Date().toISOString();
  const meta: Provenance = { source: "CHILD Act bills (Congress.gov)", kind: "api", live: true, fetchedAt: generatedAt };

  if (!dbConfigured) {
    return degraded(
      { generatedAt, configured: false, count: 0, byTier: { core: 0, related: 0, tangential: 0 }, bills: [], note: NOTE },
      "research store not connected",
      meta,
    );
  }
  ...
  for (const c of field) {
    ...
    for (const b of bills) {
      ...
      const key = billKey(b);
      const subject: ChildActSubject = { slug: c.slug, name: c.name, relation: b.relation };
