...
// The shared "case for change" board — rendered both in the staff Peace Room
// (/dashboard/peace-room) and the public community hub (/community). It contains
// NO private campaign data, so it is safe for supporters and partners alike. Per
// documented priority it pairs the change Matt stands for (faithful to
// platform.md via ISSUE_AXES) with a SOURCED indicator that it's time for change.
//
...
export async function CaseForChange() {
  ...
  return (
    <>
      <p className="mt-4 max-w-3xl text-slate">
        This is the shared case — where the campaign, supporters, allied candidates, and partners stand
        together: the data shows it&apos;s time for a change in MO-02. Each priority below pairs the change
        we&apos;re working toward with a sourced indicator of why now. Nothing here is a guess — where a
        feed isn&apos;t connected yet, it says so.
      </p>
        ...
        {ISSUE_AXES.map((axis) => {
          ...
          return (
            <section key={axis.id} className="bg-paper p-6">
              ...
              <p className="mt-4 text-xs font-semibold text-ink">The change we&apos;re working toward</p>
