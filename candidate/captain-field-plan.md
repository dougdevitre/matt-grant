...
```mermaid
flowchart TD
    CM["Campaign Manager /<br/>Field Director"] --> RC["Regional Coordinators<br/>(by county area)"]
    RC --> CAP["Captains by role<br/>(7 roles)"]
    CAP --> VOL["Volunteer teams<br/>(5-10 each)"]
    VOL --> SUP["Supporters → Sponsors →<br/>Donors → Ambassadors"]
    SUP -->|"loop"| CAP
```
