...
```mermaid
flowchart TD
    A[Pick a Matt Grant asset from /media] --> B{What are you making?}
    B -->|Documents, flyers, palm cards| C[Mobile Printing — any branch]
    B -->|Signs, banners, swag, video| D[Creative Lab — Clark Family Branch]
    C --> C1[Register email once: print@slcl.org]
    C1 --> C2[Email file to print+bw@ or print+color@slcl.org]
    C2 --> C3[Release at any print station within 48 hrs]
    D --> D1[Call ahead to reserve / confirm equipment]
    D1 --> D2[Bring approved materials; staff assist on-site]
    D2 --> D3[Design on Adobe Macs → print/cut/record]
    C3 --> E[Use responsibly — see rules below]
    D3 --> E
```
