...
|---|---|---|---|
| **3 — Ranking core** | Pure `buildSendList()` ranks an opted-in, voter-scored audience highest-value first and caps to the budget (reuses `SMS_PRIORITY`); GOTV `excludeBanked`, per-segment restriction, `touches`/cost math. **Wired into the send path 2026-07-20:** every broadcast now queues via `rankForBroadcast()` — highest-likelihood voters first (segment weight + turnout tie-break from the denormalized consent tags), unscored last but never dropped, with an optional composer "Max texts" cap that trims only the lowest-priority tail and reports it. | `web/lib/reports/smsTargeting.ts` (+ test) → `web/app/dashboard/sms/actions.ts` | ✅ **shipped** |
| **1 — Enrichment job** | For each opted-in phone, match its named contact (volunteer/donor name+ZIP) → voter row (`matchPhones`, `score.ts`) and ballot-returns; write `voterSegment`/`voterT`/`banked` — plus `county`/`zip` geography — onto the `SMSCONSENT` row. Idempotent (`--dry-run` supported, counts-only output); re-run nightly in the chase window. Self-reported geography from the SMS vote agent (`geoSource: "self"`) is never overwritten. Pure merge core in `web/lib/reports/smsEnrichment.ts` (+ test). | `web/scripts/enrich-sms-audience.ts` (`npm run enrich:sms`) | ✅ **shipped 2026-07-20** |
| **2 — Composer targeting** | New targeting dimension (`county:<key>`, `zip:<zip5>`, `segment:MOBILIZE`, `outstanding`) resolved from the **denormalized consent fields only** — composer chips + live counts (`smsTargetCounts`). Tokens only NARROW the opted-in audience; invalid tokens are ignored. (`t>=N` deferred — segment chips cover the practical cases.) | `web/lib/sms/audiences.ts` | ✅ **shipped 2026-07-20** |
| **4 — GOTV chase mode** | During the chase window, target only **outstanding** (not-yet-`banked`) high-propensity opted-ins — don't spend texts on people who already voted. Driven by the Phase-1 `banked` tag; unenriched rows are never dropped by the outstanding filter. | "Not yet voted" composer chip | ✅ **shipped 2026-07-20** |
| **6 — Priority + budget in the composer** | Surface the segments as a first-class **"Who to reach — by likelihood to vote"** preset dropdown (`SMS_PRIORITY_PRESETS` → the same `segment:`/`outstanding` tokens as Phase 2, so no new send-path grammar) instead of a buried "narrow" chip, and add an inline **Budget $** field that computes the "Max texts" cap live (`spendModel`, the Spend Decider's pure model) and shows how far the money reaches. Preset shows always with a disabled/zero empty state before enrichment. TCPA wall preserved — the composer client imports only `lib/reports/smsSpend` (outside `lib/sms/`), never `lib/voters`. | `web/lib/sms/audiences.ts` (presets) · `web/components/dashboard/SmsComposer.tsx` · `web/app/dashboard/sms/page.tsx` | ✅ **shipped 2026-07-23** |
| **7 — Second-source targeting** | A vendor/party-committee export lands as an OVERLAY (`VOTEROVL`, keyed by official Voter ID) carrying what the Sunshine-law file lacks: real August-primary history and an **inferred** party. The enrichment job denormalizes them onto consent rows as `voterPp` / `voterParty`, and the composer gains `pp:<0-5>` (a MINIMUM) and `party:<code>` chips plus an "August-primary regulars, not yet voted" preset. Absent ≠ zero: an unenriched row is dropped by these filters rather than treated as a primary non-voter. TCPA wall untouched — the party-code list in `audiences.ts` is a mirrored string literal, not an import, and a test asserts it can't drift. | `web/lib/voters/sources/`, `web/lib/voters/overlayStore.ts`, `web/scripts/inspect-voter-source.ts`, `web/scripts/ingest-voter-overlay.ts` → `web/lib/sms/audiences.ts` — see [`voter-registry-refresh-plan.md`](./voter-registry-refresh-plan.md) | ✅ **tooling shipped 2026-07-26** (awaiting the source file) |
