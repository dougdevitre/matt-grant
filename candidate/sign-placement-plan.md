...
|---|---|---|---|
| **0. Foundations** | Now → Jul 12 | Confirm boundary, data, inventory | _[ ]_ |
| **1. Data & scoring** | Jul 10–15 | Build ranked location list | _[ ]_ |
| **2. Field org & turf** | Jul 12–20 | Captains, volunteers, turf packets | _[ ]_ |
...
- **Do:** hard-filter candidates (in-district, permission obtainable, buffer-legal), then score and rank
  every surviving location (§8); allocate by phase/type.
...
```csv
# placement_output.csv  (generated by the Phase 1 scoring pass)
rank,name,lat,lng,type,phase,captain_id,assigned_volunteer,score,precinct,aadt,notes
1,"Daniel Boone Library",38.6031,-90.5673,library_early_vote,A,C01,V001,0.94,"STL-042",,"14-day captive audience"
```
...
`Daniel Boone Library` — early-vote+E-Day site: `voter_contact_value 0.9 × days_active(norm) × visibility 0.9 × serviceability 1.0` → top-ranked (Phase A), because 14 days of confirmed-voter exposure beats a one-day poll or a corridor impression. Numbers are placeholders.
