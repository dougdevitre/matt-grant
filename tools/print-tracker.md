...
- **`disclaimer_required = Y`** — the piece is a public political communication and needs a "Paid for by the Matt Grant for Congress Committee" disclaimer. Generate the exact text and per-medium placement (print font ≥ the readable minimum) with `tools/disclaimer-generator.md`. The mailing generators in `tools/pdf-letterhead/` already render this footer on every page.
